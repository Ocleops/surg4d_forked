...
python cluster.py -s  data/hypernerf/${dataset_name} --language_features_name ${language_feature_name} --model_path output/hypernerf/${dataset_name} \
    --feature_level ${level} --skip_train --skip_test --configs arguments/hypernerf/chicken.py --mode ${mode} --no_dlang 1 --load_stage fine-lang --num_views 5 --autoencoder_ckpt_path autoencoder/ckpt/${dataset_name}_clip/best_ckpt.pth
done
done
